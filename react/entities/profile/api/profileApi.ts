...
/**
 * [신규] 현재 로그인한 사용자의 프로필이 DB에 존재하는지 확인합니다.
 * @returns { success: boolean, exists: boolean }
 */
export const checkProfileExistsAPI = async (): Promise<{ success: boolean; exists: boolean; }> => {
    const response = await fetch(`${API_BASE_URL}/exists`, {
        credentials: 'include'
    });
    return handleApiResponse<{ success: boolean; exists: boolean; }>(response);
};
